...
@Check_invite.route('/admin/item/check_invite',methods=['POST'])
def check_invite():
    ...
    sql = "select project_name,invitecode from project where project_key = any (select project_key from people_join where user_admin=1 and user_ID = '%s');" % (user_ID)
    cursor.execute(sql)     
